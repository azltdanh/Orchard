...
$(function () {
    ...
    $('table.items tbody tr td input[type=checkbox]').change(function () {
        $(this).closest('tr').toggleClass('selected', $(this).is(':checked'));
    });
    $('.selectAll').change(function () {
        $('input:checkbox[name^=' + this.value + ']').attr('checked', this.checked).change();
    });
    ...
    $('#AdsTypeCssClass').change(function () {
        if ($(this).val() == 'ad-leasing') $('#PaymentMethodId').val(57).change();else $('#PaymentMethodId').val(56).change();
            ...
            $.ajax({
                ...
                success: function success(response) {
                    ...
                    $.each(response.list, function (i, item) {
                        $('#TypeId').append("<option value=" + item.Value + ">" + item.Text + "</option>");
                    });
    ...
    $('#Floors').change(function () {
        $.ajax({
            ...
            success: function success(response) {
                ...
                $.each(response.list, function (i, item) {
                    $('#TypeConstructionId').append("<option value=" + item.Value + ">" + item.Text + "</option>");
                });
                ...
                success: function success(response) {
                    ...
                    $.each(response.apartmentList, function (i, item) {
                        if (item.Path != '') {
                            ...
                            $("ul.gallery").append('<li class="apartment_' + apartmentId + '"> \
                    ...
                    if (response.apartmentWardId == 0) slcWard.siblings('.combobox-container').children('input').val('');else $('#WardId').val(response.apartmentWardId).combobox('refresh').change();

                    if (response.apartmentStreetId == 0) slcStreet.siblings('.combobox-container').children('input').val('');else $('#StreetId').val(response.apartmentStreetId).combobox('refresh').change();
    ...
    $('#Published').change(function () {
        if ($(this).is(':checked')) $('#AddAdsExpirationDate').val('Day90');
    });
    ...
    // BĐS nổi bật
    $('#AdsHighlight').change(function () {
        if ($(this).is(':checked')) $('#AddAdsHighlightExpirationDate').val('Day90');
    });
    ...
    // Price change
    $('#PriceProposed, #PaymentMethodId, #PaymentUnitId').change(function () {
        DocGiaRao();
    });

    $('#PriceProposed').keyup(function () {
        DocGiaRao();
    });
    ...
    $("#WardId, #StreetId, #ApartmentId, #AddressNumber, #AddressCorner, #ApartmentNumber, #AdsTypeCssClass").change(function () {
        if (!$('#IsExternal').is(':checked')) {
            ...
            if (districtId > 0 && (wardId > 0 && streetId > 0 && addressNumber != "" && addressNumber != null || apartmentId > 0 && apartmentNumber != "" && apartmentNumber != null)) {
                $.ajax({
                    type: "post",
                    dataType: "",
    ...
    $('#tabsAdsType a').click(function () {
        ...
        if ($(this).is(':contains("thuê")')) $('#PaymentMethodId option:contains("Triệu"):first').attr('selected', 'selected');else $('#PaymentMethodId option:contains("Tỷ")').attr('selected', 'selected');
    ...
    $("form[method=get]").submit(function () {
        ...
        $('input[type=text], select, input[type=hidden]', this).each(function () {
            this.disabled = $(this).val() == 'false' || $(this).val() == '';
        });
    ...
    $("form").submit(function () {
        ...
        $('input[type="submit"], .button').prop('disabled', true).click(function () {
            return false;
        });
    });
    ...
    if ($.fn.datepicker) {
        $('.date-box').datepicker({
            ...
            onSelect: function onSelect(selectedDate) {
                ...
                if (elId.indexOf('From') >= 0) $('#' + $(this).attr('id').replace('From', 'To')).datepicker('option', 'minDate', selectedDate);else if (elId.indexOf('To') >= 0) $('#' + $(this).attr('id').replace('To', 'From')).datepicker("option", "maxDate", selectedDate);
            }
        });
    }
...
$('#FloorFrom, #FloorTo').change(function () {
    ...
    if (floorFrom > 0 && floorTo > 0) {
        if (floorTo - floorFrom < 0) alert('Giá trị nhập vào không đúng!');
...
$(function () {
    ...
    if ($.fn.select2) {
        ...
        var formatSelection = function formatSelection(item) {
            return item.name;
        };
        ...
        var initSelection = function initSelection(element, callback) {
            var id = $(element).val();
            if (id !== "") {
                // User
                if (element.is('.select2-user')) $.ajax("/RealEstate.Admin/Home/GetUserForJson/" + id, { dataType: "json" }).done(function (data) {
                    callback(data);
                });

                // Group
                if (element.is('.select2-group')) $.ajax("/RealEstate.Admin/Home/GetGroupForJson/" + id, { dataType: "json" }).done(function (data) {
                    callback(data);
                });
            }
        };
        ...
        $('.select2-user,.select2-group').select2({
            ...
            ajax: {
                url: function url() {
                    ...
                    // User
                    if ($(this).is('.select2-user-available-group-member')) return "/RealEstate.Admin/Home/SearchAvailableGroupMemberUsersForJson";
                    if ($(this).is('.select2-user-available-group-admin')) return "/RealEstate.Admin/Home/SearchAvailableGroupAdminUsersForJson";
                    if ($(this).is('.select2-group-user')) return "/RealEstate.Admin/Home/SearchGroupUsersForJson";
                    if ($(this).is('.select2-user')) return "/RealEstate.Admin/Home/SearchUsersForJson";

                    // Group
                    if ($(this).is('.select2-group-seeder')) return "/RealEstate.Admin/Home/SearchSeederGroupsForJson";
                    if ($(this).is('.select2-group')) return "/RealEstate.Admin/Home/SearchGroupsForJson";
                },
                dataType: 'json',
                type: 'GET',
