...
$(function () {
    ...
    $(".dropdown-check-list").dropdownchecklist({ icon: {}, width: _defaultWidth, maxDropHeight: _maxDropHeight, emptyText: "-- Tất cả --", groupItemChecksWholeGroup: true });

    if ($('#DistrictIds option').length == 0) { disableDropdownChecklist($("#DistrictIds")); } else { initDropdownChecklist($("#DistrictIds")); }
    if ($('#WardIds option').length == 0) { disableDropdownChecklist($("#WardIds")); } else { initDropdownChecklist($("#WardIds")); }
    if ($('#StreetIds option').length == 0) { disableDropdownChecklist($("#StreetIds")); } else { initDropdownChecklist($("#StreetIds")); }
        ...
        if ($(this).val() > 0) {
            $.ajax({
                ...
                success: function (response) {
                    $("#DistrictIds").empty().dropdownchecklist("destroy");
                    $.each(response.list, function (i, item) { $("#DistrictIds").append("<option value=" + item.Value + ">" + item.Text + "</option>"); });
                    if (selectedDistrictIds != null) $("#DistrictIds").val(selectedDistrictIds);
                    initDropdownChecklist($("#DistrictIds"));
                    $("#DistrictIds").change();

                },
                error: function (request, status, error) {
                    $("#DistrictIds").empty().change();
                    initDropdownChecklist($("#DistrictIds"));
    ...
    $("#DistrictIds").change(function () {
        if ($(this).val()) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    $("#WardIds").empty().dropdownchecklist("destroy");
                    $.each(response.list, function (i, item) { $("#WardIds").append("<option value=" + item.Value + ">" + item.Text + "</option>"); });
                    if (selectedWardIds != null) $("#WardIds").val(selectedWardIds);
                    initDropdownChecklist($("#WardIds"));
                },
                error: function (request, status, error) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $("#StreetIds").empty().dropdownchecklist("destroy");
                    $.each(response.list, function (i, item) { $("#StreetIds").append("<option value=" + item.Value + ">" + item.Text + "</option>"); });
                    if (selectedStreetIds != null) $("#StreetIds").val(selectedStreetIds);
                    initDropdownChecklist($("#StreetIds"));

                    $("#StreetId").empty().append("<option value=''></option>");
                    $.each(response.list, function (i, item) { $("#StreetId").append("<option value=" + item.Value + ">" + item.Text + "</option>"); });
                    enableCombobox($("#StreetId"));

                },
                error: function (request, status, error) {
                    disableDropdownChecklist($("#StreetIds"));
                    $("#StreetId").empty().append("<option value=''></option>");
                    disableCombobox($("#StreetId"));
                }
            });
            return false;
        }
    ...
    $("#resetDistrictIds").click(function () {
        $("#selectedDistrictIds option").removeAttr("selected");
        $("#selectedDistrictIds").dropdownchecklist("refresh");
        $("#selectedDistrictIds").change();
    })
    $("#resetWardIds").click(function () {
        $("#selectedWardIds option").removeAttr("selected");
        $("#selectedWardIds").dropdownchecklist("refresh");
        $("#selectedWardIds").change();
    })
    $("#resetStreetIds").click(function () {
        $("#selectedStreetIds option").removeAttr("selected");
        $("#selectedStreetIds").dropdownchecklist("refresh");
        $("#selectedStreetIds").change();
    })
});

var _maxDropHeight = 1000;
var _defaultWidth = 140;

function initDropdownChecklist(el) {
    var _title = "-- Tất cả --";
    if (el.attr("id") == "DistrictIds") _title = "Chọn tất cả Quận / Huyện";
    if (el.attr("id") == "WardIds") _title = "Chọn tất cả Phường / Xã";
    if (el.attr("id") == "StreetIds") _title = "Chọn tất cả các Đường";
    $(el).dropdownchecklist({ icon: {}, width: _defaultWidth, maxDropHeight: _maxDropHeight, emptyText: _title });
}
function disableDropdownChecklist(el) {
    $(el).empty();
    $(el).dropdownchecklist("destroy");
    initDropdownChecklist($(el));
