...
namespace RM.QuickLogOn.OAuth.Services
{
    ...
    public class GoogleOAuthService : IGoogleOAuthService
    {
        ...
        public GoogleOAuthService(IEncryptionService oauthHelper, IQuickLogOnService quickLogOnService, IOrchardServices services)
        {
        ...
        private  string GetAccessToken(WorkContext wc, string code)
        {
            try
            {
                ...
                using (var ws = new StreamWriter(stream, Encoding.UTF8))
                {
                    ...
                    ws.Write("grant_type={0}", "authorization_code");
                }
                //Services.Notifier.Error(T("host: {0} - ContentLength: {1} - AuthenticationLevel: {2}",host, wr.ContentLength, wr.AuthenticationLevel));

                HttpWebResponse wres = (HttpWebResponse)wr.GetResponse();// wr.GetResponse();
            ...
            catch (WebException ex)
            {
                ...
                if (ex.Status == WebExceptionStatus.ProtocolError)
                {
                    ...
                    if (err != null)
                    {
                        ...
                        Services.Notifier.Error(T("Error: {0} - Response: {1}", ex.Message, txtResults));
                    }
                }
            }
