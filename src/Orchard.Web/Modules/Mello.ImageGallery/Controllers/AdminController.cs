...
namespace Mello.ImageGallery.Controllers {
    public class AdminController : Controller {
        ...
        {
            ViewBag.IsManageImageGallery = Services.Authorizer.Authorize(Permissions.ManageImageGallery);/*add*/
        ...
        public ActionResult Images(string imageGalleryName) {
            if (!Services.Authorizer.Authorize(Permissions.ManageUpdateImageGallery, T("Cannot edit image gallery")))//old => ManageImageGallery
            {
                return new HttpUnauthorizedResult();
            }

            ViewBag.IsManageImageGallery = Services.Authorizer.Authorize(Permissions.ManageImageGallery);
        ...
        public ActionResult AddImages(string imageGalleryName) {
            if (!Services.Authorizer.Authorize(Permissions.ManageUpdateImageGallery, T("Cannot add images to image gallery")))//old => ManageAddImageGallery
            {
                return new HttpUnauthorizedResult();
            }
        ...
        public ActionResult AddImages(ImageAddViewModel viewModel) {
            if (!Services.Authorizer.Authorize(Permissions.ManageUpdateImageGallery, T("Couldn't upload media file")))//old => ManageImageGallery
            {
                return new HttpUnauthorizedResult();
            }
            ...
            try {
                if (viewModel.ImageFiles == null || !viewModel.ImageFiles.Any() || viewModel.ImageFiles.First() == null) {
                ...
                //Disable other file /*add*/
                _imageGalleryService.UpdateIsPublishFile(viewModel.ImageGalleryName);
        ...
        public ActionResult EditImage(string imageGalleryName, string imageName) {
            if (!Services.Authorizer.Authorize(Permissions.ManageUpdateImageGallery, T("Cannot edit image")))//old => ManageImageGallery
            {
                return new HttpUnauthorizedResult();
            }
        ...
        public ActionResult EditImage(ImageEditViewModel viewModel) {
            ...
            try {
                ...
                if (viewModel.Image.Enable && (string.IsNullOrEmpty(viewModel.Image.DateBegin) || string.IsNullOrEmpty(viewModel.Image.DateEnd)))
                {
                    ModelState.AddModelError("NullDateExpired", T("Vui lòng chọn ngày bắt đầu và ngày hết hạn quảng cáo của bạn!").ToString());
                    return
                        View(new ImageEditViewModel
                        {
                            ImageGalleryName = viewModel.ImageGalleryName,
                ...
                try {
                    _imageGalleryService.UpdateImageProperties(viewModel.ImageGalleryName, viewModel.Image.Name,
                                                               viewModel.Image.Title, viewModel.Image.Caption, viewModel.Image.href, viewModel.Image.DateBegin, viewModel.Image.DateEnd, viewModel.Image.Enable, viewModel.Image.Blank, viewModel.Image.IsPublish);
                }
                catch (Exception exception) {
        ...
        #region Flash Swf Create
        [HttpGet]
        public ActionResult FlashCreate(string imageGalleryName)
        {
            ...
            return
                View(new ImageEditViewModel { ImageGalleryName = imageGalleryName });
        }
        [HttpPost]
        //[FormValueRequired("submit.Save")]
        public ActionResult FlashCreate(ImageEditViewModel viewModel)
        {
            if (!Services.Authorizer.Authorize(Permissions.ManageImageGallery, T("Cannot edit image")))
            {
                return new HttpUnauthorizedResult();
            ...
            try
            {
                ...
                try
                {
                    _imageGalleryService.UpdateFlashProperties(viewModel.ImageGalleryName, viewModel.Image.Name,
                                                               viewModel.Image.Title, viewModel.Image.Caption, null, viewModel.Image.href, viewModel.Image.DateBegin, viewModel.Image.DateEnd, viewModel.Image.Enable, viewModel.Image.Blank, viewModel.Image.IsPublish);
                }
                catch (Exception exception)
                {
                    Services.Notifier.Error(T("Editing image properties failed: {0}-{1}-{2}-{3}-{4}-{5}-{6}-{7}-{8}", exception.Message, viewModel.ImageGalleryName, viewModel.Image.Name,
                                                               viewModel.Image.Title, viewModel.Image.Caption, viewModel.Image.href, viewModel.Image.DateBegin, viewModel.Image.DateEnd, viewModel.Image.IsPublish));
                    return
                        View(new ImageEditViewModel
                        {
                            ImageGalleryName = viewModel.ImageGalleryName,
                ...
                Services.Notifier.Information(T("Image properties successfully modified{0}, {1}", viewModel.Image.DateBegin, viewModel.Image.DateEnd));
            }
            catch (Exception exception)
            {
                Services.Notifier.Error(T("Saving image failed: {0}", exception.Message));
            ...
            return RedirectToAction("FlashCreate", new { imageGalleryName = viewModel.ImageGalleryName });//Images
        }
