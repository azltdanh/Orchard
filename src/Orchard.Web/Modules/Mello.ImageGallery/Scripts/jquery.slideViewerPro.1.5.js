...
jQuery.fn.slideViewerPro = function(settings) {
	...
	return this.each(function(){
		...
		*/
		var pictWidth = container.find("img").attr("width");//width();
		var pictHeight = container.find("img").attr("height");//height();
    ...
    .wrap(jQuery("<div style='width:"+ pictWidth +"px; overflow: hidden; position: relative; top: 0; left: 0'>"));
		...
		container.each(function(i) {
			...
			var rbttLeftMargin = (pictWidth / 2) + (jQuery("div#thumbSlider" + j).attr("width") / 2) + settings.thumbsRightMargin + contBorder;
			var lbttLeftMargin = (pictWidth / 2) - (jQuery("div#thumbSlider" + j).attr("width") / 2) - (settings.buttonsWidth + settings.thumbsRightMargin) + contBorder;
			...
			if (settings.autoslide) {
			    ...
			    //stops autoslidemode  
					...
					//});
                // stop slide
					$("#svwp" + j).bind("mouseover", function () {
					    jQuery(this).next().children(".thumbSlider").stopTime("asld");
					});

					$("#svwp" + j).bind("mouseout", function () {
					    jQuery(this).next().children(".thumbSlider").everyTime(settings.asTimer, "asld", function () {
					        jQuery(this).find("a").eq(i).trigger("click");
					        if (i == 0) {
					            pos = 0;
					            l_enabled = false;
		...
		(jQuery("div#thumbSlider" + j).attr("width") + (settings.buttonsWidth * 2) >= pictWidth) ? alert(jQuery("div#thumbSlider" + j).attr("width") + " + " + settings.buttonsWidth * 2 + " = " + pictWidth + " ALERT: THE THUMBNAILS SLIDER IS TOO WIDE! \nthumbsPercentReduction and/or buttonsWidth needs to be scaled down!") : null;
		j++;
  });	
};